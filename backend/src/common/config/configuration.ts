...
const environmentSchema = z
  .object({
    ...
    REFRESH_TOKEN_TTL_SECONDS: integerString(3_600, 7_776_000).default(
      2_592_000,
    ),
    REFRESH_COOKIE_NAME: z
      .string()
      .regex(/^[A-Za-z0-9_-]+$/)
      .default("buzzerhood_refresh"),
    CSRF_COOKIE_NAME: z
      .string()
      .regex(/^[A-Za-z0-9_-]+$/)
      .default("buzzerhood_csrf"),
  ...
  .superRefine((value, context) => {
    if (value.DATABASE_POOL_MIN > value.DATABASE_POOL_MAX) {
      context.addIssue({
        ...
        message: "Pool minimum cannot exceed maximum.",
      });
    }
    ...
    const smtpValues = [
      ...
      value.SMTP_PASSWORD,
      value.SMTP_FROM,
      value.PARTNER_APPLICATION_NOTIFICATION_EMAIL,
    ];
    if (
      smtpValues.some(Boolean) &&
      smtpValues.some((smtpValue) => !smtpValue)
    ) {
      context.addIssue({
        code: "custom",
        path: ["SMTP_HOST"],
        message: "SMTP configuration must be complete when email is enabled.",
      });
    }
  });
...
export function loadConfiguration(
  ...
  return {
    ...
    rateLimit: {
      ttlMs: parsed.AUTH_RATE_LIMIT_TTL_MS,
      max: parsed.AUTH_RATE_LIMIT_MAX,
    },
    email: {
      enabled: Boolean(parsed.SMTP_HOST),
