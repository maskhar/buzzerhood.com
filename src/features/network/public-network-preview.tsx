...
import networkDatabaseHero from '@/assets/images/network-database-hero.png';
...
export function PublicNetworkPreview({ previewLimit }: { previewLimit?: number } = {}) {
  ...
  return <section className="db-section" id="database"><div className="wrap"><div className="db-side reveal"><div className="section-head"><p className="eyebrow">NETWORK DATABASE — LIVE</p><h2>Cari langsung dari jaringan aktif</h2><p>Contoh potongan database publik Buzzerhood. Cari nama, handle, atau niche, lalu saring berdasarkan platform dan tier.</p></div><div className="photo-tile"><img src={networkDatabaseHero} alt="Jaringan media, creator, komunitas, dan buzzer Buzzerhood" loading="lazy" /><span className="cap">Buzzerhood Network</span></div></div><div className="db-toolbar"><label className="db-search"><span className="label">Search</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Cari nama, handle, niche..." /></label><div className="db-filter-selects"><label className="filter-group db-select-group"><span>Tier</span><select value={activeTier} onChange={(event) => setActiveTier(event.target.value as (typeof tiers)[number])}>{tiers.map((tier) => <option key={tier} value={tier}>{tier}</option>)}</select></label><label className="filter-group db-select-group"><span>Platform</span><select value={activePlatform} onChange={(event) => setActivePlatform(event.target.value as (typeof platforms)[number])}>{platforms.map((platform) => <option key={platform} value={platform}>{platform}</option>)}</select></label></div></div>{!hasApiConfig ? <p className="db-empty">API belum dikonfigurasi pada environment ini.</p> : null}{network.isLoading ? <LoadingState /> : null}{network.isError ? <p className="db-empty">Network database belum tersedia. Periksa API dan migration database.</p> : null}{network.isSuccess ? <><p className="db-meta"><strong>{formatNumber(filteredRows.length)}</strong> akun ditemukan dari <strong>{formatNumber(rows.length)}</strong> total di network database · Total metric: <strong>{formatNumber(totalReach)}</strong></p><div className="db-table-wrap"><table className="db-table"><thead><tr><th>Nama</th><th>Platform</th><th>Tier</th><th>Metric</th><th>Niche</th><th>Type</th></tr></thead><tbody>{filteredRows.slice(0, previewLimit ?? filteredRows.length).map((partner: PublicNetworkPartner) => <tr key={partner.id}><td><strong>{partner.display_name}</strong><span>{partner.handle}</span></td><td>{partner.platform}</td><td><span className="tier-badge">{partner.tier}</span></td><td>{formatNumber(partner.metric_value ?? 0)} {partner.metric_type ? metricLabels[partner.metric_type] ?? partner.metric_type : ''}</td><td><span className="niche-tag">{partner.niche}</span></td><td><span className="partner-badge">{partner.partner_type}</span></td></tr>)}</tbody></table>{filteredRows.length === 0 ? <p className="db-empty">Tidak ada data yang cocok dengan filter.</p> : null}</div>{previewLimit ? <Link className="db-see-more" to="/database">Lihat Database Lengkap →</Link> : null}</> : null}</div></section>;
}
