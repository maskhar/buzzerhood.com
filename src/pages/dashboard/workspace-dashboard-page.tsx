...
export function WorkspaceDashboardPage({ kind }: { kind: WorkspaceKind }) {
  ...
  return <section className="workspace-dashboard-page">
    <header className="workspace-hero"><div><p className="eyebrow">{labels[kind]} DASHBOARD</p><h1>Selamat datang,<br/><span>{displayName}</span></h1><p>Kelola aktivitas {labels[kind].toLowerCase()} dari satu workspace terpusat.</p></div><div className="workspace-status"><span className="status-dot"/><div><strong>Workspace aktif</strong><small>Backend API terhubung</small></div></div></header>
